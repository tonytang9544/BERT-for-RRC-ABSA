...
def train(data_folder, run_folder, do_validation=True, model_path="bert-base-uncased", epochs=6, batch_size=32, learn_rate=5e-5, seed=0):
    ...
    for epoch in range(epochs):
        ...
        for batch in dataloader:
            ...
            pred_start = torch.matmul(F.softmax(start_logits, dim=-1, dtype=float), torch.arange(start_logits.shape[1], dtype=float).to(device))
            pred_end = torch.matmul(F.softmax(end_logits, dim=-1, dtype=float), torch.arange(end_logits.shape[1], dtype=float).to(device))
